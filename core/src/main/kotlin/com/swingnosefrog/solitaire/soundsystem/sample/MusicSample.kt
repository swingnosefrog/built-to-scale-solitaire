package com.swingnosefrog.solitaire.soundsystem.sample
...
abstract class MusicSample(
    ...
    protected val startBuffer: Buffer by lazy(LazyThreadSafetyMode.PUBLICATION) {
        Buffer(
            this,
            msToSamples(DEFAULT_START_BUFFER_MS).toInt()
        )
    }
    protected val playbackBuffer: Buffer by lazy(LazyThreadSafetyMode.PUBLICATION) {
        Buffer(
            this,
            msToSamples(DEFAULT_PLAYBACK_BUFFER_MS).toInt()
        )
    }
...
//    }
    ...
    class Buffer(val musicSample: MusicSample, val samples: Int, val bytesPerSample: Int = 2) {
        ...
        var size: Int = 0
            private set
        private val tmpBuffer: ByteBuffer =
            ByteBuffer.allocate(256 * bytesPerSample * nChannels) // Buffer size must be a multiple of bytesPerSample
                ...
                while (bytesRead > 0) {
                    ...
                    for (i in 0..<samplesReadable) {
                        for (ch in 0..<nChannels) {
                            data[ch][i + samplesRead] =
                                ((tmpBuffer.get(tmpPtr).toInt() and 0xFF) or (tmpBuffer.get(tmpPtr + 1)
                                    .toInt() shl 8)) / 32_768.0f
