...
open class NinepatchPane : Pane() {
    ...
    override fun renderSelf(originX: Float, originY: Float, batch: SpriteBatch) {
        ...
        if (texture != null) {
            ...
            // Corners
            batch.drawUV(
                texture, x, y - cornerHeight, cornerWidth, cornerHeight,
                0f, 0f, third, third
            )
            batch.drawUV(
                texture, x + w - cornerWidth, y - cornerHeight, cornerWidth, cornerHeight,
                2 * third, 0f, 1f, third
            )
            batch.drawUV(
                texture, x, y - h, cornerWidth, cornerHeight,
                0f, 2 * third, third, 1f
            )
            batch.drawUV(
                texture, x + w - cornerWidth, y - h, cornerWidth, cornerHeight,
                2 * third, 2 * third, 1f, 1f
            )

            // Sides
            batch.drawUV(
                texture, x + cornerWidth, y - cornerHeight, innerWidth, cornerHeight,
                third, 0f, 1f - third, third
            )
            batch.drawUV(
                texture, x + cornerWidth, y - h, innerWidth, cornerHeight,
                third, 1f - third, 1f - third, 1f
            )
            batch.drawUV(
                texture, x, y - h + cornerHeight, cornerWidth, innerHeight,
                0f, third, third, 1f - third
            )
            batch.drawUV(
                texture, x + w - cornerWidth, y - h + cornerHeight, cornerWidth, innerHeight,
                1f - third, third, 1f, 1f - third
            )

            // Centre
            batch.drawUV(
                texture, x + cornerWidth, y - h + cornerHeight, innerWidth, innerHeight,
